...
* `Ref` and `RefPtr` - These smart pointers are used like [`std::shared_ptr`](https://en.cppreference.com/w/cpp/memory/shared_ptr). They extend the lifetime of an object so long as there is an outstanding `Ref` or `RefPtr`. To use `Ref` or `RefPtr` with an object, make the class inherit from `RefCounted<T>` or define `ref()` and `deref()` functions which implement the semantics of `RefCounted`. RetainPtr works similarly for Objective-C objects. OSObjectPtr work similarly for Darwin OS objects.
